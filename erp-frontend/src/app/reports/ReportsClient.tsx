...
import { FabricVarianceRow, Style, Supplier, WastageRejectionReport } from "@/lib/api";
import { fmtQty } from "@/lib/format";
import { Card, PageHeader, Table, Td, Th } from "@/components/ui";
import { useERP } from "@/lib/useERP";

const EMPTY_WASTAGE: WastageRejectionReport = { wastage_by_style: [], rejection_by_vendor: [], scrapped_by_style: [] };
...
export default function ReportsClient({ token }: { token: string }) {
  const { data: variance = [] } = useERP<FabricVarianceRow[]>("/reports/fabric-variance", token);
  const { data: wastage = EMPTY_WASTAGE } = useERP<WastageRejectionReport>("/reports/wastage", token);
  const { data: styles = [] } = useERP<Style[]>("/styles", token);
    ...
    <main className="max-w-5xl mx-auto px-8 py-10">
      ...
      {variance.length === 0 ? (
        ...
        <Table>
          <thead>
            <tr>
              <Th>Order</Th><Th>Style</Th><Th>Lot</Th><Th>Planned</Th><Th>Actual</Th><Th>Variance</Th><Th>Wastage</Th>
            </tr>
          </thead>
          <tbody>
            {variance.map((row) => (
              <tr key={row.cutting_record_id}>
                ...
                <Td className={Number(row.variance_qty) > 0 ? "text-destructive font-medium" : ""}>{fmtQty(row.variance_qty)}</Td>
